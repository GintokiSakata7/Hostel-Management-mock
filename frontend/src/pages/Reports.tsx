...
const printTable = (title: string, headers: string[], rows: any[][], subtitle = '') => {
  ...
  win.document.write(`
    ...
    <div class="footer">VMR Hostel Management System · Generated on ${new Date().toLocaleString('en-IN')}</div>
    </body></html>`);
...
function StudentReport() {
  ...
  const handlePrint = () => {
    printTable('Student Report', ['Name', 'Course', 'Branch', 'Year', 'Roll No.', 'Room', 'Bed', 'Phone', 'Admission Date'],
      filtered.map(s => [s.name, s.course, s.branch, s.year, s.rollNumber, s.room, s.bedNumber, s.phone, s.admissionDate]));
  };
            ...
            {loading ? <tr><td colSpan={6} style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-muted)' }}>Loading…</td></tr>
              : filtered.map((s, i) => (
                <tr key={i}>
                  ...
                  <td>{s.course}{s.branch ? ` · ${s.branch}` : ''}</td>
                  <td>{s.year} Year</td>
                  <td>{s.room} {s.bedNumber ? `/ Bed ${s.bedNumber}` : ''}</td>
                  <td style={{ color: 'var(--text-muted)', fontSize: '0.82rem' }}>{s.phone || '—'}</td>
                  <td style={{ color: 'var(--text-muted)', fontSize: '0.82rem' }}>{s.admissionDate || '—'}</td>
                </tr>
              ))}
          </tbody>
        </table>
...
function RoomReport() {
  ...
  const handlePrint = () => {
    printTable('Room Occupancy Report', ['Building', 'Floor', 'Room', 'Capacity', 'Occupied', 'Available', 'Occupancy %'],
      filtered.map(r => [r.building, r.floor, r.room, r.capacity, r.occupied, r.available, `${r.occupancyPct}%`]));
  };
            ...
            {loading ? <tr><td colSpan={7} style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-muted)' }}>Loading…</td></tr>
              : filtered.map((r, i) => (
                <tr key={i}>
                  ...
                  <td>{r.capacity}</td>
                  <td style={{ color: r.occupied > 0 ? 'var(--danger)' : 'var(--text-muted)' }}>{r.occupied}</td>
                  <td style={{ color: r.available > 0 ? 'var(--success)' : 'var(--text-muted)' }}>{r.available}</td>
                  <td>
...
function FeeReport({ pendingOnly = false }: { pendingOnly?: boolean }) {
  ...
  const handlePrint = () => {
    ...
    if (pendingOnly) {
      printTable(title, ['Student', 'Course', 'Room', 'Bed', 'Pending Amount'], filtered.map(s => [s.name, s.course, s.room, s.bedNumber, `₹${s.amount}`]), subtitle);
    } else {
      printTable(title, ['Student', 'Course', 'Room', 'Bed', 'Amount', 'Status', 'Date', 'Mode', 'Ref ID'],
        filtered.map(s => [s.name, s.course, s.room, s.bedNumber, `₹${s.amount}`, s.feeStatus, s.paymentDate, s.method, s.transactionRef]), subtitle);
    }
  };
                ...
                <tr key={i}>
                  ...
                  {!pendingOnly && (
                    <>
                      ...
                      <td><span className={`fee-status-chip ${s.feeStatus === 'Paid' ? 'fee-paid' : 'fee-pending'}`} style={{ fontSize: '0.68rem' }}>{s.feeStatus}</span></td>
                      <td style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>{s.paymentDate || '—'}</td>
                      <td style={{ fontSize: '0.8rem' }}>{s.method || '—'}{s.upiProvider ? ` (${s.upiProvider})` : ''}</td>
                      <td style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>{s.transactionRef || '—'}</td>
                    </>
                  )}
                  {pendingOnly && <td style={{ fontWeight: 700, color: 'var(--warning)' }}>₹{(5500).toLocaleString()}</td>}
                </tr>
              ))}
          </tbody>
        </table>
...
function ComplaintReport() {
  ...
  const handlePrint = () => {
    printTable('Complaint Report', ['Title', 'Student', 'Room', 'Priority', 'Status', 'Date'],
      filtered.map(c => [c.title, c.student, c.room, c.priority, c.status, c.date]));
  };
        ...
        <table className="custom-table" style={{ width: '100%' }}>
          ...
          <tbody>
            {loading ? <tr><td colSpan={6} style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-muted)' }}>Loading…</td></tr>
              : filtered.map((c, i) => (
                <tr key={i}>
                  <td><strong style={{ fontSize: '0.88rem' }}>{c.title}</strong>{c.description && <div style={{ color: 'var(--text-muted)', fontSize: '0.74rem', marginTop: 2 }}>{c.description.slice(0, 60)}…</div>}</td>
                  <td>{c.student}</td>
                  <td>{c.room}</td>
                  <td><span style={{ padding: '3px 8px', borderRadius: 12, fontSize: '0.68rem', fontWeight: 700, background: c.priority === 'high' ? 'rgba(239,68,68,0.12)' : c.priority === 'medium' ? 'rgba(234,179,8,0.12)' : 'rgba(34,197,94,0.12)', color: c.priority === 'high' ? 'var(--danger)' : c.priority === 'medium' ? 'var(--warning)' : 'var(--success)' }}>{c.priority}</span></td>
                  <td><span className={`status-badge badge-${c.status === 'resolved' ? 'available' : c.status === 'in-progress' ? 'partial' : 'full'}`}>{c.status}</span></td>
